...
export const aiUatApi={
  ...
  loadTest:(payload:LoadTestRequest)=>request<LoadTestResult>('/api/performance/load-test',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}),
  loadTestHistory:()=>request<LoadTestRun[]>('/api/performance/history'),
  automationScripts:(companyId:string,productId:string)=>request<AutomationScript[]>(`/api/automation-scripts?companyId=${encodeURIComponent(companyId)}&productId=${encodeURIComponent(productId)}`),
  createAutomationScript:(payload:{companyId:string;productId:string;name:string;steps:string[]})=>request<AutomationScript>('/api/automation-scripts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}),
  approveAutomationScript:(id:string)=>request<AutomationScript>(`/api/automation-scripts/${id}/approve`,{method:'PATCH'}),
  reviseAutomationScript:(id:string,steps:string[])=>request<AutomationScript>(`/api/automation-scripts/${id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({steps})}),
  generateAutomationScript:(id:string,payload:{url?:string;expectedResult?:string})=>request<GeneratedAutomation>(`/api/automation-scripts/${id}/generate`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}),
  traceability:(companyId:string,productId:string)=>request<TestTraceability[]>(`/api/test-management/traceability?companyId=${encodeURIComponent(companyId)}&productId=${encodeURIComponent(productId)}`),
  createTraceability:(payload:{companyId:string;productId:string;requirementId:string;testCondition:string;testCaseId:string;automationScriptId?:string;riskLevel:string;expectedResult:string;entryCriteriaMet:boolean})=>request<TestTraceability>('/api/test-management/traceability',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}),
  recordTraceabilityExecution:(id:string,payload:{executionId:string;status:string;actualResult:string;defectRef?:string})=>request<TestTraceability>(`/api/test-management/traceability/${id}/execution`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}),
  testCompletionSummary:(companyId:string,productId:string)=>request<TestCompletionSummary>(`/api/test-management/summary?companyId=${encodeURIComponent(companyId)}&productId=${encodeURIComponent(productId)}`)
};
