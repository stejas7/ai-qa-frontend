...
export default function ExecutionCenterPage(){
  ...
  return <main className="page"><div className="eyebrow">M4 • ADVANCED AUTOMATION</div><h1>Execution Center</h1><p className="lead">Registered UAT applications, Playwright execution metrics and persisted evidence from the Spring Boot backend.</p>
    ...
    <section className="panel"><div className="section-heading"><div><div className="eyebrow">AUDIT TRAIL</div><h2>Execution Evidence</h2></div><span className="live">● Live</span></div><div className="stack-list">{history.data?.slice(0,30).map((x,i)=><div className="list-card" key={x.id||`${x.testId}-${i}`}><div><strong>{x.testId}</strong><small>{x.targetUrl} • {x.durationMs}ms{x.diagnosticMessage?` • ${x.diagnosticMessage}`:''}</small></div><div><span className={`status ${x.status==='PASS'?'completed':'failed'}`}>{x.status}</span>{x.screenshot&&<a className="view-link evidence-link" href={x.screenshot} target="_blank" rel="noreferrer">Evidence</a>}</div></div>)}{history.data?.length===0&&<p className="muted">No executions yet.</p>}</div></section></div>
  </main>
}
