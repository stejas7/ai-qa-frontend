...
export default function ApiReferencePage(){
  ...
  return <main className="page api-page">
    ...
    <section className="metric-grid four compact">
      ...
      <article><strong>{groups.length}</strong><span>API domains</span></article>
      <article><strong>REST + JSON</strong><span>Primary contract</span></article>
      <article><strong>/actuator/health</strong><span>Deployment health</span></article>
    </section>

    <section className="panel api-note">
      <div className="eyebrow">LIVE BASE URL</div>
      <h2>https://auravis-uat.duckdns.org</h2>
      <p className="muted">React is served by Nginx. Requests under <code>/api/*</code> and <code>/actuator/*</code> are proxied to the Spring Boot backend running on EC2.</p>
    </section>

    {groups.map(group=><section className="panel" key={group.title}>
      <div className="section-heading"><div><div className="eyebrow">API DOMAIN</div><h2>{group.title}</h2><p className="muted">{group.description}</p></div><span className="status completed">{group.endpoints.length} endpoints</span></div>
      <div className="table-wrap"><table className="api-table"><thead><tr><th>Method</th><th>Endpoint</th><th>Purpose</th><th>Milestone</th></tr></thead><tbody>{group.endpoints.map(endpoint=><tr key={`${endpoint.method}-${endpoint.path}`}><td><span className={methodClass(endpoint.method)}>{endpoint.method}</span></td><td><code>{endpoint.path}</code></td><td>{endpoint.purpose}</td><td><span className="tech-chip">{endpoint.milestone}</span></td></tr>)}</tbody></table></div>
    </section>)}

    <section className="panel boundary-panel"><div className="eyebrow">ENGINEERING BOUNDARY</div><h2>Why this API layer matters</h2><p className="lead">Auravis keeps AI reasoning, React presentation and deterministic system actions separated. React talks to documented REST endpoints; Java owns policy and persisted state; Playwright performs allowed browser actions; PostgreSQL stores evidence and history. The API contract is therefore part of the product architecture, not hidden implementation detail.</p></section>
  </main>
}
